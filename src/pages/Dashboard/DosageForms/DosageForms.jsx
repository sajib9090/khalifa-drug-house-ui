...
import AddNewDosageForm from "../../../components/DosageForm/AddNewDosageForm";
import {
  useGetAllDosageFormsQuery,
  useRemoveDosageFormMutation,
} from "../../../redux/features/dosageForm/dosageFormApi";
...
const DosageForms = () => {
  ...
  const { data: dosageForms, isLoading } = useGetAllDosageFormsQuery({
  ...
  const [removeDosageForm, { isLoading: deleteLoading }] =
    useRemoveDosageFormMutation();
  const handleRemoveConfirm = async () => {
    if (selectedId) {
      try {
        const res = await removeDosageForm(selectedId).unwrap();
        if (res?.success) {
          setIsOpen(false);
          setSelectedId(null);
        ...
        <div className="flex justify-between">
          <div>
            <div className="text-[18px] font-bold capitalize">
              ...
              <Link to={"/dashboard/dosage-forms"}>
        ...
        <>
            ...
            <table className="w-full min-w-[500px]">
              <thead>
                <tr className="border-b border-[#d0cfcf] h-[35px] w-full text-[14px] sm:text-[16px]">
                  ...
                  <th className="w-[70%] text-start">Dosage Form</th>
          ...
          <div className="flex items-center justify-between mt-2 mb-4">
            <div>
              ...
              <select
                disabled={dosageForms?.data_found <= 10}
                value={resultPerPage}
                onChange={(e) => {
            ...
            <div className="flex items-center">
              <button
                ...
                disabled={
                  dosageForms?.pagination?.previousPage == null || isLoading
                }
              ...
              <button
                onClick={() => setPage(page + 1)}
                disabled={
                  dosageForms?.pagination?.nextPage == null || isLoading
                }
                className={`h-[27px] w-[27px] flex items-center justify-center border border-gray-300 rounded ${
